...
def tt_combi_Subj(token):
  ...
  if (token.n_rights==0 and token.n_lefts==0):
    ...
    combi_droite=[]
    for leftchild in token.lefts:
      combi_gauche=cartesian_product(combi_gauche,tt_combi_Obj(leftchild))
    for rightchild in token.rights:
      combi_droite=cartesian_product(combi_droite,tt_combi_Obj(rightchild))
...
def visiterVerbe(token):
  ...
  for child in token.rights:
    if (child.dep_=="conj"):
      clause_list=clause_list+visiter_regle(token,child,["nsubj"],["conj"],["NOUN"])# (nsubj)  (verb) (conj vers NOUN)
    if (child.dep_=="dobj"):
      clause_list=clause_list+visiter_regle(token,child,["nsubj"],["dobj"],["NOUN"]) # (nsubj)  (verb) (dobj vers NOUN)
    if (child.dep_=="prep"):
      clause_list=clause_list+visiter_regle(token,child,["nsubjpass"],["prep"],["ADP"]) #nsubjpass (VERB) prep vers ADP
      clause_list=clause_list+visiter_regle(token,child,["nsubjpass"],["prep"],["SCONJ"]) #nsubjpass (VERB) prep vers SCONJ
    if (child.dep_=="attr"): 
      clause_list=clause_list+visiter_regle(token,child,["expl","nsubj"],["attr"],["NOUN"]) # PRON<-expl|nsubj- AUX -attr->NOUN
  return clause_list
